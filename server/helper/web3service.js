...
/**
 * Transfers land on-chain using DMalpot ERC-721 signature:
 * transferLand(uint256 _landId, address _newOwner)
 */
export const transferLandOnChain = async (
  ...
  try {
    ...
    if (!sellerCitizenshipNo || typeof sellerCitizenshipNo !== "string") {
      throw new Error(`Invalid seller citizenship number: '${sellerCitizenshipNo}'`);
    }

    if (!buyerCitizenshipNo || typeof buyerCitizenshipNo !== "string") {
      throw new Error(`Invalid buyer citizenship number: '${buyerCitizenshipNo}'`);
    }

    // Convert landId and price to BigInt safely for Ethers v6 / Viem
    const parsedLandId = BigInt(landId);
    const parsedPrice = BigInt(priceInWei || 0);

    // 2. Pre-flight Check: Verify if Land is Frozen on-chain before attempting tx
    const landData = await contract.getLand(parsedLandId);
    if (landData.isFrozen) {
      throw new Error(`Land ID ${landId} is currently frozen (Roka) on-chain.`);
    }

    // 3. Execute On-Chain Transfer (4 Parameters)
    ...
    throw new Error(`On-chain land transfer failed: ${reason}`);
  }
};
...
export const updateLandCategoryOnChain = async (landId, newCategory) => {
  try {
    if (landId === undefined || landId === null || isNaN(Number(landId))) {
      throw new Error(`Invalid or missing landId parameter: received '${landId}'`);
    }

    // Convert string category to enum index if passed as string
    let categoryEnum = 0;
    if (typeof newCategory === "number" && !isNaN(newCategory)) {
      ...
      if (index === -1) {
        throw new Error(
          `Invalid category string: '${newCategory}'. Must be one of: ${categories.join(", ")}`
        );
      }
      categoryEnum = index;
    }
    ...
    const contract = getDMalpotContract();
    const tx = await contract.setLandCategory(Number(landId), categoryEnum);
    const receipt = await tx.wait();

    return receipt.hash;
  } catch (error) {
    throw new Error(`On-chain land category update failed: ${error.message}`);
  }
};